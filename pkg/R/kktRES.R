...
##
## Method for determining residuals of a KKT-system for (nonlinear, epigraph-form) convex programs with nonlinear constraints
setMethod("kktRES", signature = c("DEFCP", "PDV", "PDV"), function(cpd, u, v, W, sdv){
    idx <- 1:cpd@k
    ## dual residual
    ## v@x := v@x - H * ux - A'*u@y - G' * W^{-1} * u@z
    Hx <- drop(crossprod(cpd@H, u@x[1:cpd@n]))
    Ay <- drop(crossprod(cpd@A[,1:cpd@n], u@y))
    Winvz <- lapply(idx, function(j) usnt(u@z[[j]]@u, W[[j]], inv = TRUE, trans = FALSE))
    GWinvz <- Reduce("+", lapply(idx, function(j) drop(crossprod(cpd@cList[[j]]@G, Winvz[[j]]@u))[1:cpd@n]))
    v@x[1:cpd@n] <- v@x[1:cpd@n] - Hx - Ay - GWinvz
    ## primal residual
    ## v@y := v@y - A * u@x 
    v@y <- v@y - drop(cpd@A %*% u@x)
    ## centrality residual (dual)
