    ...
    for(i in 0:(ctrl@maxiters + 1)){
        ...
        if((pres <= ctrl@feastol) & (dres <= ctrl@feastol) &
           (gap <= ctrl@abstol || checkRgap)){
            ...
            CurSol@z <- CurPdv@z
            if(mnl > 1){
                CurSol@s[[1]]@u <- matrix(CurSol@s[[1]]@u[-1, 1])
                CurSol@s[[1]]@dims <- CurSol@s[[1]]@dims - 1L
                CurSol@z[[1]]@u <- matrix(CurSol@z[[1]]@u[-1, 1])
                CurSol@z[[1]]@dims <- CurSol@z[[1]]@dims - 1L
            } else {
                CurSol@s <- CurSol@s[-1]
                CurSol@z <- CurSol@z[-1]
            }
        ...
        for(ii in 0:1){
            ...
            ## unscaling slack-variables
            dsu <- lapply(idx, function(j) usnt(SolKkt@s[[j]]@u, W[[j]], inv = FALSE, trans = TRUE))
            dsu <- lapply(idx, function(j) usnt(SolKkt@z[[j]]@u, W[[j]], inv = TRUE, trans = FALSE))
            ## Maximum step to boundary
            SolKkt@s <- lapply(idx, function(j) uslb(u = SolKkt@s[[j]], lambda = W[[j]]@W[["lambda"]]))
            SolKkt@z <- lapply(idx, function(j) uslb(u = SolKkt@z[[j]], lambda = W[[j]]@W[["lambda"]]))
            ...
            for(j in idxPSD){
                ...
                dim(Qs) <- c(SolKkt@s[[j]]@dims^2, 1)
                SolKkt@s[[j]]@u <- Qs
                Qz <- MaxStepZ[[j]]$evd$vectors
                dim(Qz) <- c(SolKkt@z[[j]]@dims^2, 1)
                SolKkt@z[[j]]@u <- Qz
            }          
            ...
            while(backtrack){
                NewPdv <- CurPdv
                NewPdv@x <- NewPdv@x + step * SolKkt@x
                NewPdv@y <- NewPdv@y + step * SolKkt@y
                NewPdv@z <- lapply(idx, function(j){
                    ans <- dsu[[j]]
                    ans@u <- NewPdv@z[[j]]@u + step * dsu[[j]]@u
                    ans
                })
                NewPdv@s <- lapply(idx, function(j){
                    ans <- dsu[[j]]
